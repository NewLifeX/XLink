...
namespace xLinkServer.Controllers
{
    public abstract class BaseController : ControllerBase, IActionFilter
    {
        #region 属性
        /// <summary></summary>
        private static readonly TokenSession _session = new TokenSession();
        ...
        protected Device _deviceForHistory;
        #endregion
        ...
        public void OnActionExecuted(ActionExecutedContext context)
        {
            if (context.Exception != null)
            {
                ...
                var dv = Session?["Device"] as Device ?? _deviceForHistory;
                WriteHistory(dv, action, false, context.Exception?.GetTrue() + "");
            }
        }
        ...
        private static TokenProvider GetTokenProvider(Boolean generate)
        {
            if (_tokenProvider != null) return _tokenProvider;

            var provider = new TokenProvider();
            provider.ReadKey("../keys/token.prvkey", generate);
        ...
        private IDictionary<String, Object> RestoreSession(String token)
        {
            ...
            var code = rlist[0];
            var dv = Device.FindByCode(code) ?? new Device { Code = code };
            ...
            if (!provider.TryDecode(token, out var result, out var dt))
            {
                ...
                WriteHistory(dv, "签名错误", false, msg);
            ...
            if (dt < DateTime.Now)
            {
                ...
                XTrace.WriteLine(msg);
                WriteHistory(dv, "令牌过期", false, msg);
            ...
            WriteHistory(dv, "令牌复活", true, $"[{code}]{token}");
